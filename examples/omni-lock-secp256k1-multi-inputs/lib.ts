...
export interface TransferOptionsArray extends Array<TransferOptions> {};
...
export async function buildTransfer(options: TransferOptionsArray) {
  let tx = helpers.TransactionSkeleton({});
  let totalAmount:number = 0;

  for (const opt of options) {
    totalAmount += parseInt(opt.amount);
  }
  ...
  const transferOutput: Cell = {
    cell_output: {
      capacity: BI.from(totalAmount).toHexString(),
      lock: helpers.parseAddress(options[0].to),
    },
    data: "0x",
  };

  tx = tx.update("outputs", (outputs) => outputs.push(transferOutput));

  for (const opt of options) {
    console.log(opt)
    const fromScript = helpers.parseAddress(opt.from);
    // const toScript = helpers.parseAddress(opt.to);

    // additional 1 ckb for tx fee
    // the tx fee could calculated by tx size
    // this is just a simple example
    const neededCapacity = BI.from(opt.amount).add(100000000);
...
export async function signByPrivateKey(txSkeleton: helpers.TransactionSkeletonType, privateKey: string, addressIndex: number) {
  ...
  txSkeleton = txSkeleton.update("witnesses", (witnesses) => witnesses.set((addressIndex - 1), signedWitness));
